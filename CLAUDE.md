...
- **Relay a la resta de pantalles de jugador**: quan el DM rep un `TOKEN_MOVE` vàlid (BC o WS, handlers a `DMView.tsx`), després d'actualitzar `rPos`/`setPos` crida `_broadcastState({})` perquè els altres clients (p. ex. una tele que mostra `/player`) rebin la nova posició en temps real. Sense això, només s'actualitzaven en fer refresh de la pantalla del DM. El client que arrossega conserva la seva posició local (handler `STATE` del jugador), així que el relay no el fa saltar.
