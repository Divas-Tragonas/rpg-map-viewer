...
- **Relay a la resta de pantalles de jugador**: quan el DM rep un `TOKEN_MOVE` vàlid (BC o WS, handlers a `DMView.tsx`), després d'actualitzar `rPos`/`setPos` crida `_broadcastState({}, { omitCamera: true })` perquè els altres clients (p. ex. una tele que mostra `/player`) rebin el moviment en temps real. Sense això, només s'actualitzaven en fer refresh de la pantalla del DM. **Important sobre `omitCamera`**: el relay ha d'enviar l'estat **complet** (vis, grid, i camps pesats si han canviat) perquè no desaparegui res del jugador — un relay que només porti `pos` deixa la resta de l'estat sense refrescar i provoca que enemics/sales desapareguin. Alhora **no ha d'incloure la càmera** (`zoom`, `panOffset`, `dmPreview*`): un STATE amb càmera forçaria la vista del client i li resetejaria el zoom a cada moviment de token. Per això `_broadcastState` accepta `opts.omitCamera`. El client que arrossega conserva la seva posició local (handler `STATE` del jugador), així que el relay no el fa saltar.
