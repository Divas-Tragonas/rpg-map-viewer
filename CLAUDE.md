...
- **Amplada per dos clics** (`nearestWallHit` + `doorEndOnWall` a `src/lib/rooms/doors.ts`): la col·locació és de **dos clics**. **Primer clic** → marca l'inici de la porta imantat a la paret més propera (`rDoorPlacement.current.anchor = { wall, s }`). **Segon clic** → fixa el final (l'amplada) projectat sobre la **mateixa paret** i crea la porta (`addDoor`). Amb grid, cada extrem s'imanta a la línia de graella més propera al llarg de l'eix dominant de la paret (`snapSToGrid`). No hi ha límit de mida ni control `+/−` (deprecats: `rDoorWidthCells` i `doorPlacementAt` queden sense ús).
