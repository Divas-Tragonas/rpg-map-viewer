...
- **Límit de moviment per velocitat** (`Player.speed`, peus; per defecte `DEFAULT_SPEED_FT = 30`): el drag des de `/player` clampa el centre del token a un rang de `floor(speed/5)` caselles al voltant de la casella d'origen del drag. **El cost de moviment tracta la diagonal com a 10 ft** (2 × ortogonal, com la variant D&D), de manera que la regió abastable és un **rombe** (distància de Manhattan: `|dc| + |dr| ≤ maxCells`), no un quadrat — una forma de moviment coherent en comptes d'una que arriba massa lluny en diagonal. El clamp projecta el punter sobre la vora del rombe conservant la direcció (`onPointerMove` a `usePlayerTokenDrag.ts`). Les caselles abastables es pinten en groc suau durant el drag amb perímetre esglaonat (`renderMoveRange` a `render/grid.ts`, cridada al tick del jugador abans dels tokens). El DM edita la velocitat al `PlayersPanel` (`setPlayerSpeed`) i **no** té cap límit en moure tokens. `speed` viatja dins l'array `players` (STRUCT/STATE/sessió), sense camps de sync nous.
