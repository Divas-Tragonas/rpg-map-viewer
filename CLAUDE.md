...
- **Obrir/tancar**: amb l'eina Parets (sense cadena en curs), **clic sobre una porta la commuta** (`toggleDoor`). Les portes noves neixen obertes.
- **Amplada variable** (`rDoorWidthCells`, 1–10 caselles, per defecte 1): durant la col·locació, **+/− canvia l'amplada** (1 casella = `gridSize` px, o 60 px sense grid; mai més del 90% de la paret). El valor es recorda per a la porta següent. El snap s'adapta a la paritat: amplada senar → centre de casella; amplada parella → frontera entre caselles (la porta sempre cobreix caselles exactes).
- **Col·locació obligada**: en tancar-se una sala nova amb l'eina Parets, s'obre automàticament el **mode de col·locació de porta** (`rDoorPlacement`): mentre mous el cursor, la previsualització (`rDoorPreview`, càlcul a `doorPlacementAt`) llisca **sempre imantada a la paret més propera**, amb snap addicional a la graella (eix dominant de la paret) quan hi ha grid, i sense sortir mai de la paret. Clic → es col·loca (`addDoor`) i surt del mode; **Maj+clic → la col·loca i continua en mode porta** (per posar diverses entrades seguides); **Esc → s'omet**. El rètol d'ajuda (amb l'amplada en peus) es pinta a `renderDoorDraft` (espai pantalla, `useRafLoop` després de `renderWallDraft`).
...
- **Render** (`renderWalls`): el traç sòlid de paret usa les parets efectives (el forat es veu de veritat) i cada porta es marca amb un **rectangle groc de traç gruixut i buit per dins** (`drawDoorMark`) muntat sobre la paret; amb la porta tancada la línia de paret travessa el rectangle i s'hi afegeix un farciment tènue. Només al DM.
...
- **Model**: cada token de jugador **visible i no derrotat** emet llum dins del seu radi de visió (`Player.visionFt`, peus; per defecte `DEFAULT_VISION_FT = 30`, `0` = sense llum; el DM l'edita al desplegable del `PlayersPanel` via `setPlayerVision`). Com `speed`, viatja dins l'array `players` sense camps de sync nous.
- **Render** (`renderRooms` a `src/lib/render/darkrooms.ts`): la foscor de les sales fosques es pinta en una **capa fora de pantalla** (`_darkCanvas`, reutilitzada; mateixa transformació que el ctx via `getTransform`), s'hi **esborren** les zones il·luminades amb `destination-out` i es composita el resultat. Cada llum s'esborra amb el **polígon de visibilitat** (raycasting per escombrat angular contra les parets, `src/lib/rooms/visibility.ts` → `visibilityPolygon`) com a clip + un **gradient radial** (la sala "es va il·luminant" amb la distància; radi = `visionFt/5 × gridSize`). Totes les llums s'acumulen primer a una **capa de llum** (`_lightCanvas`) que es composita amb `filter: blur` (`LIGHT_BLUR_PX`, px de pantalla) perquè els **marges de la llum quedin difuminats** (si el canvas del navegador no suporta `filter` — iOS Safari vell — es composita sense blur). Les parets bloquegen la llum; s'escola per les portes i obertures (es raytraça contra les **parets efectives**, amb els forats de porta retallats). Sense fonts de llum es pinta directe (camí barat, com abans). El centre de la llum segueix `visualPosRef` (LERP) perquè es mogui fluida.
