...
- **Render** (`renderRooms` a `src/lib/render/darkrooms.ts`): la foscor de les sales fosques es pinta en una **capa fora de pantalla** (`_darkCanvas`, reutilitzada; mateixa transformació que el ctx via `getTransform`), s'hi **esborren** les zones il·luminades amb `destination-out` i es composita el resultat. Cada llum s'esborra amb el **polígon de visibilitat** (raycasting per escombrat angular contra les parets, `src/lib/rooms/visibility.ts` → `visibilityPolygon`) com a clip + un **gradient radial** (la sala "es va il·luminant" amb la distància; radi animat, veure a dalt). L'alpha del gradient s'escala per la **intensitat** de la llum (0..1) per a l'esvaïment en morir. El gradient radial manté la **brillantor plena fins al 82% del radi** i només s'esvaeix a l'últim tram (així una sala més petita que el radi queda del tot il·luminada fins a les parets). Les llums s'acumulen a una **capa de llum** (`_lightCanvas`) que es composita amb `destination-out`. La llum s'atura **EXACTAMENT a les parets** (el polígon de visibilitat les respecta): **res de blur ni d'engreix** — abans feien vessar un marge de llum a la sala del costat (fuga a bordes adjacents). La vora exterior suau la dóna només el gradient radial. Les parets bloquegen la llum; s'escola per les portes i obertures (es raytraça contra les **parets efectives amb obertura de porta animada**, `effectiveWallsAnimated`). Sense fonts de llum es pinta directe (camí barat, com abans). El centre de la llum segueix `visualPosRef` (LERP) perquè es mogui fluida.
- **Boira "explorada" (AoE)** (`_exploredCanvas`, `rFogExplored` sync, default ON): les zones ja vistes es mantenen amb foscor **més clara** (saps el terreny però no hi veus enemics). Els polígons de visibilitat s'acumulen a una **màscara en coords de mapa** (resolució reduïda, `EXPLORED_MAX`) que guarda directament el color `EXPLORED_COLOR` (opac); en compositar, es repinta amb `source-atop` (només dins de les sales fosques ja pintades, **sense revelar els tokens per sota** — la foscor explorada és OPACA, només més clara), abans d'esborrar la llum actual del tot (`destination-out`). La màscara persisteix per mapa (es reinicia quan canvia la mida) i és **local a cada pantalla** (un jugador que reconnecta reexplora). El DM el commuta al `BottomControls` (🌫️) i se sincronitza dins `STATE`/`STRUCT` (camp lleuger `fogExplored`).
...
- **Moviment vorejant les parets (forma d'L)** (`rMovePath`, `computePath` a `src/lib/rooms/pathing.ts`): amb grid+parets, en deixar anar es calcula el **camí real** (Dijkstra amb predecessors, mateixes regles que `computeReachableCells`) de l'origen del drag al destí i es desa a `rMovePath[id]` com a llista de punts. `renderPlayerTokens` (branca jugador) recorre aquests punts amb el **mateix ease-out que el LERP recte** (progrés per longitud d'arc `t += (total − t)·TOKEN_LERP`; `pts` inclou l'origen), així un moviment de diverses caselles és igual de lent i suau que abans però resseguint la L, i acaba exacte al destí; la **llum no talla per sales fosques que el token no travessa**. Sense grid/parets, o si origen i destí són adjacents, no es desa camí → LERP recte de sempre. També s'aplica a la resta de pantalles via `TOKEN_RELAY` (veure sota): cada pantalla recalcula el camí des de la posició antiga (`_setMovePath` a `PlayerView`; un eco del relay del propi token, origen==destí, és no-op i no esborra el camí ja fixat). El handler de `STATE` del jugador conserva la posició local del token arrossegat perquè un broadcast del DM no el faci saltar enrere.
