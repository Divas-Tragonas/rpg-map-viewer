...
- **Relay a la resta de pantalles de jugador**: quan el DM rep un `TOKEN_MOVE` vàlid (BC o WS, handlers a `DMView.tsx`), després d'actualitzar `rPos`/`setPos` reenvia un STATE **només amb `pos`** (`{ type: 'STATE', pos }` per BC i WS) perquè els altres clients (p. ex. una tele que mostra `/player`) rebin la nova posició en temps real. Sense això, només s'actualitzaven en fer refresh de la pantalla del DM. **Important: el relay NO ha d'incloure `zoom`/`panOffset`** — un STATE complet (`_broadcastState`) forçaria la vista del client i li resetejaria el zoom cada cop que es mou un token. El client que arrossega conserva la seva posició local (handler `STATE` del jugador), així que el relay no el fa saltar.
