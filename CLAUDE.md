...
- **Moviment vorejant les parets (forma d'L)** (`rMovePath`, `computePath` a `src/lib/rooms/pathing.ts`): amb grid+parets, en deixar anar es calcula el **camí real** (Dijkstra amb predecessors, mateixes regles que `computeReachableCells`) de l'origen del drag al destí i es desa a `rMovePath[id]` com a llista de punts. `renderPlayerTokens` (branca jugador) recorre aquests punts a **velocitat de creuer lenta i constant** (`PATH_CRUISE` × mida de casella per frame) amb **frenada suau** al final (`PATH_BRAKE`, ease-out); `pts` inclou l'origen i acaba exacte al destí. Molt lent i cinematogràfic; la **llum no talla per sales fosques que el token no travessa**. Els moviments d'1 casella no desen camí → LERP recte de sempre (ràpid). Sense grid/parets, o si origen i destí són adjacents, no es desa camí → LERP recte de sempre. També s'aplica a la resta de pantalles via `TOKEN_RELAY` (veure sota): cada pantalla recalcula el camí des de la posició antiga (`_setMovePath` a `PlayerView`; un eco del relay del propi token, origen==destí, és no-op i no esborra el camí ja fixat). El handler de `STATE` del jugador conserva la posició local del token arrossegat perquè un broadcast del DM no el faci saltar enrere.
