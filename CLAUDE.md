...
- **Model** (`Door` a `types/index.ts`, helpers a `src/lib/rooms/doors.ts`): una porta és un **segment sobre una paret** (`{id, a, b}`). Per a llum i col·lisió es calculen les **parets efectives** (`effectiveWalls`): cada paret amb els seus trams de porta retallats — per la porta hi passen la llum i el moviment. La detecció de sales continua usant les parets senceres (una porta no parteix la sala). Una paret pot tenir **diverses portes**.
...
- **Render** (`renderWalls`): el traç sòlid de paret usa les parets efectives (el forat es veu de veritat) i cada porta es marca amb **brancals verds + línia discontínua** (`drawDoorMark`). Només al DM.
