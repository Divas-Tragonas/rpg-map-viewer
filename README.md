...
### v4.00
- **Redesign fase 1: targeta de criatura reconstruïda de zero** (`CreatureCard.tsx`, substitueix el JSX antic de la llista "A l'escena" de `EnemyLibraryPanel`). Estructura nova: retrat 64×64 bisellat amb el sprite a 2x; nom en VT323 codificat per amenaça (verd fàcil / or elit / vermell boss / apagat abatut, derivada de `hpMax` fins que el model porti camps reals) amb `lv{N}` a sota; vida com a **♥ + número exacte i fila de pips de cor pixel-art** (ple → esquerdat → buit, màx. 10 — mai barra contínua); i **sis ranures fixes de condició** de 20×20 amb glyphs de 8×8 dibuixats píxel a píxel (14 condicions, tooltip al hover, cap etiqueta de text). Les condicions transformen el render del sprite: petrificat = escala de grisos, enverinat = vel verd, derribat = rotació 90°, inconscient = màscara de dithering 50%, invisible = contorn puntejat + opacitat 25%, hechizado = vel magenta. L'espai mort de la capçalera s'omple amb ◆ repetits. Tota la targeta és `.bevel` sobre `--panel`, cablejada amb les condicions reals del DM (prop nova `conditions` al panell).
